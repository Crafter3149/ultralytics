...
| E1_FOCAL | B | E1 + cls focal (α.25/γ2) | E1 | — | 🚫 | | | |
| E1_NWD | B | E1 + box NWD (C13) | E1 | — | 🚫 | | | |
...
**轉向結論（2026-06-03，P2/P1 線終結 → E_NWDTAL）**：跨模型聯集分析（`diag_union_ceiling.py`）顯示 **recall 幾乎不是瓶頸**——E_FOCAL 單模型 test 偵測天花板 0.929、全模型聯集 0.946（max_det=1000 下 E_FOCAL 0.963），真正「全模型都漏」的硬底只有 ~3%（且多為低對比/怪形、非最小，見 `hard_floor/` 目視）。**E1（P2）對偵測天花板零貢獻**（聯集多 0 個），且加 P2 把 STAL 門檻 `stride[0]` 8→4、反而削弱 4–8px 保護。**瓶頸＝anchor 排序 / cls 信心，不是解析度。** 根因經 `tal.py` 核實：STAL（`select_candidates_in_gts`）已把小目標強制匹配進正樣本，但 cls 軟標籤被 `pos_overlaps`＝真實 IoU 封頂（`tal.py:138-142`）→ 小物件 IoU ~0.5-0.7 → 信心上不去 → 排序爛。**故終結 P2/P1（E1_*/E2 全 🚫），主攻 `E_NWDTAL`：封頂的 IoU 換成 NWD（平滑、對小物件不崩），只動封頂、不動選樣。** 證據工具：`diag_recall_ceiling.py` / `diag_union_ceiling.py` / `diag_size_recall.py` / `probe_hard.py` / `export_hard_floor.py`。
